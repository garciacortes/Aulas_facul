...
public class EstruturaElseIf implements RegrasSintaticas {
	...
	public boolean verificar(ArrayList<ControleSintatico> VetorAnaliseLexica) {
		...
		int comparacao = 0;
		int logico = 0;
		if(LinhaAnalisada.vetorAnalise.get(0).token() == Token.ESTRUTURAELSEIF) {
			for(int i = 1; i < LinhaAnalisada.vetorAnalise.size(); i++) {
				...
				if(LinhaAnalisada.vetorAnalise.get(i).token() == Token.ABREPARENTESE) {
					...
					System.out.println("ESTRUTURAELSEIF -");
